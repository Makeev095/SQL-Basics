...
/* Заходим в таблицу likes, оттуда в таблицу media,
в ней сравниваем media_id из likes с id из media,
потом заходим в таблицу profiles, в ней сравниваем user_id из profiles с user_id из media,
соединяем через JOIN и
подчитываем всё получившееся из запроса SELECT при условии, что пользователь младше 12 лет */

SELECT COUNT(*) FROM likes JOIN media ON likes.media_id = media.id
...
/* Подсчитываем лайки и колличество мужчин и женщин,
через JOIN соединяем user_id из likes с id из profiles,
группируем по полу и выводим в порядке убывания
*/

SELECT COUNT(likes.id) AS result, profiles.gender FROM likes
