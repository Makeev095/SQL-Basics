...
*/
USE lesson_4;
DELIMITER // -- Добавляю разделитель
CREATE TABLE users_old AS SELECT * FROM users WHERE id = 2; -- Создаю таблицу users_old

CREATE PROCEDURE user_move(IN p_user_id INT) -- Создаю процедуру
BEGIN
DECLARE EXIT HANDLER FOR SQLEXCEPTION
ROLLBACK; -- Выполняется откат изменений в слуяае ошибки
START TRANSACTION; -- Начинается транзакция
INSERT INTO users_old (id,firstname,lastname,email) -- Вставка пользователя
SELECT id, firstname, lastname, email FROM users WHERE id = user_id;
DELETE FROM users WHERE id = p_user_id; -- Удаление пользователя
COMMIT; -- Сохранение изменений
END; -- Окончание транзакции
DELIMITER; -- Закрываю разделитель
...
USE lesson_4;
DELIMITER // -- Добавляю разделитель
DROP FUNCTION IF EXISTS hello; -- Удаляю функцию при ее существовании
CREATE FUNCTION hello() -- Создаю функцию
RETURNS VARCHAR(20) -- Указываю тип возвращаемого значения
BEGIN
	DECLARE time TIME;
    DECLARE greeting VARCHAR(20);
	SET current_time = CURTIME(); -- задаю переменную времени и заполняю ее текущим
IF current_time >= "06:00:00" AND current_time < "12:00:00" THEN SET greeting = "Доброе утро";
ELSEIF current_time > "12:00:00" AND current_time < "18:00:00" THEN SET greeting = "Добрый день";
ELSEIF current_time >= "18:00:00" AND current_time < "00:00:00" THEN SET greeting = "Добрый вечер";
ELSE SET greeting = "Доброй ночи";
END IF;
RETURN greeting; -- возвращаю приветствие
END//
DELIMITER; -- закрываю разделитель

CALL hello() -- вызываю функцию

-- Error Code: 2014 Commands out of sync; you can't run this command now
-- Выскакивает почему-то следующая ошибка, пытался в интернете найти решение, но что-то так ничего и не нашел(
...
CREATE TRIGGER log_after_insert_to_users AFTER INSERT ON communities
FOR EACH ROW BEGIN
	INSERT INTO logs (tablename, extenal_id, name) VALUES('communities', NEW.id, NEW.name);
END//

-- Триггер для таблицы Messages
CREATE TRIGGER log_after_insert_to_users AFTER INSERT ON messages
FOR EACH ROW BEGIN
	INSERT INTO logs (tablename, extenal_id, name) VALUES('messages', NEW.id, NEW.name);
END//